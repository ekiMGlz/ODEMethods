...
def shooting(f, y0, yp0, sol, I, method='RK4', m=1000, tol = 1e-4, maxiter=50, fast = False, log = False):
    """
    ...
    fast : boolean, optional
        if True, only the final aproximation and error are returned. Default is False.
    log = boolean, optional
        if True, aproximations are printed as they are generated. Default is False.

    Returns
    -------
    aprox : list of doubles
        Initial values for y' generated by the Newton Method in order to satisfy the BVP.
    errors : list of doubles
        Corresponding errors for each approximation, using the method and number of steps established when function is called

    Note
    ----
    The way the aproximations are generated is inefficient due to the fact that op.newton does not return
    the number of iteration it took to converge or the in between values. 
    We considered plugging the last generated value into the Newton Method
    but it generates different results and does not converge.
    op.fsolve was not used because the solution generated wasn't precise enough, even after raising the tolerance.
    """
    F = lambda yp0 : solve(f, [y0, yp0], I, m,  method = method)[1][0][-1] - sol
    if fast:
        y_init = op.newton(F, yp0, tol = tol, maxiter=maxiter)
        return y_init, F(y_init)
    iters = 1
    aprox = []
    errors = []
    while True:
        try:
            # for some reason, op.newton does not converge immediately after the tolerance is surpassed, so a manual check was added later on.
            aprox.append(op.newton(F, yp0, tol = tol, maxiter=iters)) 
        except RuntimeError as e:
            aprox.append(float(str(e).split(' ')[-1]))
        finally:
            errors.append(abs(F(aprox[-1])))
            if log:
                print('After {} itrerations, the suggested slope is {}, which generates an error of {}'.format(iters, aprox[-1], abs(F(aprox[-1]))))
            if errors[-1]<tol:
                print('Solution found after {} iterations'.format(iters))
                break
            elif iters == maxiter:
                print('Solution not found after {} iterations'.format(maxiter))
                break
            iters+=1
    return aprox, errors
